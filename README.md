...
```mermaid
graph TD
    A[User Uploads Document] -->|Image or PDF| B[Frontend UI]
    B -->|Multipart Form Data| C[FastAPI Backend]
    C --> D{Is PDF?}
    D -->|Yes| E[PyMuPDF Extracts Images]
    D -->|No| F[Direct Processing]
    E --> G[OpenCV Preprocessing]
    F --> G
    G --> H[Tesseract OCR Extraction]
    H --> I[Raw Text Generation]
    I --> J[Regex / Parsing Logic]
    J --> K[Pydantic Validation<br/>Name, Amount, Date, ID]
    K --> L[JSON Response]
    L --> M[Frontend Renders Results]
```
